...
static bool __test_001__(void) {
	...
	ASSERT_IS_NULL((void *)array_create(SIZE_MAX / 2, 4, NULL));
	ASSERT_IS_NULL((void *)array_create(4, SIZE_MAX / 2, NULL));
	return (true);
}
...
static bool __test_002__(void) {
	...
	ASSERT_NUM_EQUAL(array_inject(a, 1, __arr__, SIZE_MAX / 4), false, "%d");
	ASSERT_NUM_EQUAL(array_inject(a, SIZE_MAX, __arr__, 0), false, "%d");
